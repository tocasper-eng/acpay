use acpay
go 
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[eep_trh]') AND type in (N'U'))
   DROP TABLE  eep_trh
GO

CREATE TABLE eep_trh(
num   bigint IDENTITY(1,1)    not null,--主流水號上對下
trno          char(10)        not null,--單據編號上對下
...
constraint pk_eep_trh_trno primary key ( trno )  
)
GO 
CREATE        INDEX in_eep_trh_menuflag ON eep_trh ( menuflag )
CREATE unique INDEX in_eep_trh_num      ON eep_trh ( num      )

GO
truncate table eep_trh 

insert into eep_trh ( trno,TRNO2,trtype,trdate,chjernoi,chjernoz)
values ( 'TR26060001','','TRA','20260630','TR26060001','Y')
select * from eep_trh 
