--USE [gemio_wms]
--GO
IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'tr_011_pos_tod_mes_itio')
    DROP TRIGGER [dbo].[tr_011_pos_tod_mes_itio]
GO
-- =====================================================================
...
-- =====================================================================
CREATE TRIGGER [dbo].[tr_011_pos_tod_mes_itio] ON [dbo].[pos_tod]
AFTER   UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        ...
        BEGIN
            RAISERROR(N'pos_tod: wareno 不可為空', 16, 1);
    ...
    SELECT
        ...
        CONVERT(char(8), i.todate, 112),
        RTRIM(i.itemno),
        RTRIM('RA-OUT'),                -- posino = wareno (來源無 posino)
        RTRIM(i.tono),
        COALESCE(                        -- plantno 解析
            NULLIF(RTRIM(i.plantno), ''),
            RTRIM(w.plantno),
