use acpay
go 
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[eep_toh]') AND type in (N'U'))
   DROP TABLE  eep_toh
GO
--主檔
CREATE TABLE eep_toh(
num   bigint IDENTITY(1,1)    not null,--主流水號上對下
tono          char(10)        not null,--單據編號上對下
...
constraint pk_eep_toh_tono primary key ( tono )  
)
GO 
CREATE        INDEX in_eep_toh_menuflag ON eep_toh ( menuflag )
CREATE unique INDEX in_eep_toh_num      ON eep_toh ( num      )
