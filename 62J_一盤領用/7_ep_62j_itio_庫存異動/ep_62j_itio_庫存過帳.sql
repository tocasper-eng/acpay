...
go
--單據核准
if exists (select name from sysobjects where name = 'ep_62j_itio')
   drop procedure ep_62j_itio 
go
 
--exec .dbo.ep_62j_zy '62j_00_0000000000004','Y' 
create procedure ep_62j_itio
(
@menuflag char(20),
@chjerno  nvarchar(99) 
)
...
USING (
    SELECT
        todate, tono, toseq, itemno, wareno, toqty, chjernoz
    FROM eep_tod
    WHERE tono = @tono
) AS src
    ON  tgt.iono     = src.tono
    AND tgt.ioseq    = src.toseq
...
THEN INSERT (
     ...
     VALUES (
        src.todate, src.tono, src.toseq, 'O', src.itemno,
        src.wareno, src.wareno, src.wareno, src.wareno, src.toqty
     )
