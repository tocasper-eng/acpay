USE cfp 
GO
DROP PROCEDURE [dbo].[ep_timer_codee]
GO
create procedure [dbo].[ep_timer_codee]
as
begin
...
select  top 1 @menuflag = menuflag from timer_codee
where ok = 'N'
and len(isnull(menuflag,''))=20 
order by  dt  

update TOP (1) timer_codee set ok='W' , dt = getdate() 
where menuflag = @menuflag and ok = 'N'

SET @sql = '.dbo.ep_' + substring(@menuflag,1,6) + '_codee_' + ' ''' + @menuflag + '''' 

EXEC sp_executesql @sql

if @@ERROR = 0   
   update TOP (1) timer_codee set ok='Y' , dt = getdate() 
