USE cfp 
GO
DROP PROCEDURE [dbo].[ep_timer_post]
GO
create procedure [dbo].[ep_timer_post]
as
begin
...
select  top 1 @menuflag = menuflag from timer_post
where ok = 'N'
and len(isnull(menuflag,''))=20 
order by  dt  

update TOP (1) timer_post set ok='W' , dt = getdate() 
where menuflag = @menuflag and ok = 'N'

SET @sql = '.dbo.ep_' + substring(@menuflag,1,6) + '_post_' + ' ''' + @menuflag + '''' 

EXEC sp_executesql @sql

if @@ERROR = 0   
   update TOP (1) timer_post set ok='Y' , dt = getdate() 
