...
DROP PROCEDURE [dbo].[ep_timer_acci]
GO
create procedure [dbo].[ep_timer_acci]
as
begin
...
select  top 1 @menuflag = menuflag from timer_acct
where ok = 'N'
and len(isnull(menuflag,''))=20 
order by  dt  

update TOP (1) timer_acci set ok='W' , dt = getdate() 
where menuflag = @menuflag and ok = 'N'

SET @sql = '.dbo.ep_' + substring(@menuflag,1,6) + '_acci_' + ' ''' + @menuflag + '''' 

EXEC sp_executesql @sql

if @@ERROR = 0   
   update TOP (1) timer_acci set ok='Y' , dt = getdate() 
