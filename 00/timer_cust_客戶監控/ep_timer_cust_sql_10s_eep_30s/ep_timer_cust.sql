USE cfp 
GO
DROP PROCEDURE [dbo].[ep_timer_cust]
GO
create procedure [dbo].[ep_timer_cust]
as
begin
...
select  top 1 @menuflag = menuflag from timer_cust
where ok = 'N'
and len(isnull(menuflag,''))=20 
order by  dt  

update TOP (1) timer_cust set ok='W' , dt = getdate() 
where menuflag = @menuflag and ok = 'N'

SET @sql = '.dbo.ep_' + substring(@menuflag,1,6) + '_cust_' + ' ''' + @menuflag + '''' 

EXEC sp_executesql @sql

if @@ERROR = 0   
   update TOP (1) timer_cust set ok='Y' , dt = getdate() 
